...
describe('Reducer', function () {
  describe('#reduce', function () {
    ...
    describe('with analysis mode set', function () {
        
      cases('should evaluate unreachable logic in text blocks', ({op, rhs, lhs, result}) => {
      ...
      cases('should evaluate unreachable logic in tags even ' +
            'when no component is provided', ({op, rhs, lhs, result}) => {
        const expression: any[] = [op, // tslint:disable-line
          ['funcall', 'lhs', {}, ['scalar', lhs]],
          ['funcall', 'rhs', {}, ['scalar', rhs]]
        ];
        ...
        const reducer = new Reducer({functions, analysisMode: true});
        const tagElement: ReducibleTagElement = {
          type: 'tag',
          name: 'foo',
          rawName: 'Foo',
          attrs: {},
          reduced: false,
          children: <ReducibleTagElement[]> [{
            type: 'tag',
            name: 'bar',
            rawName: 'Bar',
            attrs: {},
            ...
            children: <ReducibleTagElement[]> [{
              ...
              name: 'baz',
              rawName: 'Baz',
              attrs: { value: { type: 'interpolation', expression } },
              reduced: false,
              children: []
            }]
          }]
        ...
        const evaluationResult = reducer.reduce([tagElement], {});
        expect(lhsCalled).toBeTruthy();
        expect(rhsCalled).toBeTruthy();
      }, [
        { op: 'and', lhs: true, rhs: true, result: true},
